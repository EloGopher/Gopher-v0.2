...
$APILoginID = 'test';
$TransactionKey = 'test';
$GopherPostURL = 'http://localhost/Gopher-v0.2/gopherphp/save-gopher.php';
...
if (count($tracklines) > 0) {
	...
	try {
		$gopherCurl = curl_init();

		curl_setopt($gopherCurl, CURLOPT_URL, $GopherPostURL);
		curl_setopt($gopherCurl, CURLOPT_CUSTOMREQUEST, 'POST');
		curl_setopt($gopherCurl, CURLOPT_POSTFIELDS, json_encode($tracklines));
		curl_setopt($gopherCurl, CURLOPT_HTTPHEADER, $Headers);

		curl_setopt($gopherCurl, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($gopherCurl, CURLOPT_RETURNTRANSFER, true);

		$content = curl_exec($gopherCurl);
		$info = curl_getinfo($gopherCurl);
		...
		curl_close($gopherCurl);
	} catch (Exception $e) {
		$response2 = $e->getMessage();
		$response = array('request' => array('url' => $url, 'post' => $postData, 'headers' => $Headers, 'status' => $info["http_code"], 'totaltime' => $info['total_time']), 'result' => $response2);
	}
}
...
function savaDataToFile($data,$isError) {
	global $SourceFolder, $APILoginID, $TransactionKey, $GopherPostURL;

	$LogArray = array();

	if (file_exists($SourceFolder . '/gopher.log')) {
		$str = file_get_contents($SourceFolder . '/gopher.log');
		$LogArray = unserialize($str);
	}

	$LogArray[] = $data;

	if ( (count($LogArray) > 10) || ($isError) ) {
...
//		var_dump($LogArray);
			...
			curl_setopt($gopherCurl, CURLOPT_URL, $GopherPostURL);
			curl_setopt($gopherCurl, CURLOPT_CUSTOMREQUEST, 'POST');
			curl_setopt($gopherCurl, CURLOPT_POSTFIELDS, json_encode($LogArray));
			curl_setopt($gopherCurl, CURLOPT_HTTPHEADER, $Headers);
	...
	$fh = fopen($SourceFolder . '/gopher.log', 'w');
	fwrite($fh, serialize($LogArray));
	fclose($fh);
}
...
function Gopher($TrackingTag, $xValue) {
	global $PhpParentFileName;
	$data = array('TY' => 'phpvar', 'Track' => $TrackingTag, 'PFN' => $PhpParentFileName, 'VV' => $xValue, 'PHPTS' => microtime(true));
	savaDataToFile($data,false);
}
